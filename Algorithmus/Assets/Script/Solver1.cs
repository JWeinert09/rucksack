...
using UnityEngine;
public class Solver1 : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        for (int i = 9; i > 0; i--)
        {
            for (int j = 0; j < i; j++)
            {
                if (sortierung[j] < sortierung[j + 1])
                {
                    sorthelp = sortierung[j];
                    sortierung[j] = sortierung[j + 1];
                    sortierung[j + 1] = sorthelp;
                }
            }
        }
        ...
        int maxweight = 50;
        int endvalue = 0;
        if (0 <= (maxweight - weightsubtracted[0]))
        {
            maxweight = maxweight - weightsubtracted[0];
            endvalue = endvalue + valueadded[0];
            Debug.Log("Item " + item[0] + " wurde ausgewählt und der Restplatz beträgt: " + maxweight);
        }
        if (0 <= (maxweight - weightsubtracted[1]))
        {
            maxweight = maxweight - weightsubtracted[1];
            endvalue = endvalue + valueadded[1];
            Debug.Log("Item " + item[1] + " wurde ausgewählt und der Restplatz beträgt: " + maxweight);
        }
        if (0 <= (maxweight - weightsubtracted[2]))
        {
            maxweight = maxweight - weightsubtracted[2];
            endvalue = endvalue + valueadded[2];
            Debug.Log("Item " + item[2] + " wurde ausgewählt und der Restplatz beträgt: " + maxweight);
        }
        if (0 <= (maxweight - weightsubtracted[3]))
        {
            maxweight = maxweight - weightsubtracted[3];
            endvalue = endvalue + valueadded[3];
            Debug.Log("Item " + item[3] + " wurde ausgewählt und der Restplatz beträgt: " + maxweight);
        }
        if (0 <= (maxweight - weightsubtracted[4]))
        {
            maxweight = maxweight - weightsubtracted[4];
            endvalue = endvalue + valueadded[4];
            Debug.Log("Item " + item[4] + " wurde ausgewählt und der Restplatz beträgt: " + maxweight);
        }
        if (0 <= (maxweight - weightsubtracted[5]))
        {
            maxweight = maxweight - weightsubtracted[5];
            endvalue = endvalue + valueadded[5];
            Debug.Log("Item " + item[5] + " wurde ausgewählt und der Restplatz beträgt: " + maxweight);
        }
        if (0 <= (maxweight - weightsubtracted[6]))
        {
            maxweight = maxweight - weightsubtracted[6];
            endvalue = endvalue + valueadded[6];
            Debug.Log("Item " + item[6] + " wurde ausgewählt und der Restplatz beträgt: " + maxweight);
        }
        if (0 <= (maxweight - weightsubtracted[7]))
        {
            maxweight = maxweight - weightsubtracted[7];
            endvalue = endvalue + valueadded[7];
            Debug.Log("Item " + item[7] + " wurde ausgewählt und der Restplatz beträgt: " + maxweight);
        }
        if (0 <= (maxweight - weightsubtracted[8]))
        {
            maxweight = maxweight - weightsubtracted[8];
            endvalue = endvalue + valueadded[8];
            Debug.Log("Item " + item[8] + " wurde ausgewählt und der Restplatz beträgt: " + maxweight);
        }
        if (0 <= (maxweight - weightsubtracted[9]))
        {
            maxweight = maxweight - weightsubtracted[9];
            endvalue = endvalue + valueadded[9];
